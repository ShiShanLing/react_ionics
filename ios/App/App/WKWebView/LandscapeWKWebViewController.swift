//
//  LandscapeWKWebViewController.swift
//  App
//
//  Created by 石山岭 on 2024/3/20.
//
import WebKit
import UIKit

class LandscapeWKWebViewController: UIViewController,WKNavigationDelegate, WKUIDelegate {
...
//      self.wkWebview.transform = CGAffineTransform(rotationAngle: CGFloat.pi / 2)
        ...
        let tempFrame = ContentView.frame;
        print("self.ContentView.frame==", ContentView.frame)
        let transform = CGAffineTransform(rotationAngle: CGFloat.pi * 1.5)
        ContentView.transform = transform;
        //刷新布局
        ContentView.frame = CGRect(x: 0, y: 0, width: tempFrame.width, height: tempFrame.height)
        self.wkWebview.frame = CGRect(x: 150, y: 64, width: tempFrame.height-300, height: tempFrame.width-64)
        print("self.ContentView.frame==", ContentView.frame)
        
        customNavBar.frame = CGRect.init(x: 0, y: 0, width: tempFrame.height, height: 64)
        closeButton.frame = CGRect(x: tempFrame.height-100, y: 25, width: 90, height: 30)
        
        goBackButton.frame = CGRect(x: 10, y: 25, width: 70, height: 30)
        titleLabel.frame = CGRect(x: 0, y: 15, width: tempFrame.height, height: 44)
